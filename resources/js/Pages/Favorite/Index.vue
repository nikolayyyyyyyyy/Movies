...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-6 sm:py-12">
            <div v-if="favoriteMovies.data.length > 0" class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div v-if="favoriteMovies.data.length > 1" class="flex justify-end mb-6">
                    <SortMovie @sortMovies="sortMovies" :sortBy="sort" />
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
                    <MovieCard v-for="favoriteMovie in favoriteMovies.data" :key="favoriteMovie.id" :movie="favoriteMovie" />
                </div>

                <nav v-if="favoriteMovies.links.length > 0" class="mt-8 flex flex-wrap items-center justify-center gap-2"
                    aria-label="Pagination">
                    <template v-for="(link, index) in favoriteMovies.links" :key="index">
            ...
            <div v-else class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <p class="text-center text-gray-500">No favorite movies found</p>
            </div>
        </div>
    </AuthenticatedLayout>
</template>