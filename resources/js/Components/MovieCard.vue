...
const triggerRateMovie = () => {
    router.post(route('favorites.storeOrDestroy'), {
...
<template>
    <div class="flex cursor-pointer flex-col relative" @click="router.visit(route('movies.show', movie.slug))">
        <PrimaryButton v-if="$page.props.auth.user.role_id == RoleEnum.Admin"
            class="absolute bg-red-500 text-white top-2 right-2" @click.stop.prevent="showDeleteModal = true">
        ...
        <button v-if="$page.props.auth.user.role_id == RoleEnum.User"
            class="absolute bg-orange-500 text-white top-2 right-2 p-2 rounded-md hover:bg-orange-800"
